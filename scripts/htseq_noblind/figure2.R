...
cov_dat = readRDS('./data/htseq/no_blind/CoV.rds')
cov_ch = readRDS('./data/htseq/no_blind/CoV_change.rds')
cd_ratio_CI = readRDS('data/htseq/no_blind/cov_ratio_jk_CI.rds')
# cd_ratio_CI_dev = readRDS('data/processed//raw/cov_ratio_jk_CI_dev.rds')
# cd_ratio_CI_aging = readRDS('data/processed//raw/cov_ratio_jk_CI_aging.rds')
...
panel_b = top_divcon_cov_dat %>%
  ...
  ggtitle('Cd93') +
  annotate('text', x=94, y=0.20, label='Ageing', hjust=0, size = szx/pntnorm, fontface='bold') +
  annotate('text', x=95, y=0.19,
           label = parse(text = paste('rho["CoV,age"]==' ,
                                      round(filter(top_divcon_cov_cordat, period=='aging')$cor,2))),
           hjust=0, size = 5/pntnorm) +
  annotate('text', x=95, y=0.18, label = filter(top_divcon_cov_cordat,period=='aging')$cor.p2,
           parse=T, hjust=0, size = szx/pntnorm) +
  annotate('text', x=3, y=0.25, label='Development', hjust=0, size = szx/pntnorm, fontface='bold')  +
  annotate('text', x=3, y=0.24,
           label = parse(text = paste('rho["CoV,age"] ==' , 
                                      (round(filter(top_divcon_cov_cordat, period=='development')$cor,2)))),
           hjust=0, size = szx/pntnorm) +
  annotate('text', x=3, y=0.23,
           label = filter(top_divcon_cov_cordat,period=='development')$cor.p2,
           parse=T, hjust=0, size = szx/pntnorm) +
  theme_bw() +
  theme(axis.text = element_text(size=6),
        axis.title = element_text(size=6),
...
panel_a = ggplot(cov_dat_sum, aes(x = age, y= meanCoV)) +
  ...
  annotate('text',x=94,y=0.16,label='Ageing',hjust=0, size = szx/pntnorm, fontface='bold') +
  annotate('text',x = 94, y=0.155,
           label = parse(text = paste('rho["CoV,age"]==' ,(round(filter(cov_cordat,period=='aging')$cor,2)))),
           hjust=0,size = 5/pntnorm) +
  annotate('text',x = 94, y=0.150,
           label = parse(text = paste0('p ==' ,(round(filter(cov_cordat,period=='aging')$cor.p,2)))),
           hjust=0,size = szx/pntnorm) +
  annotate('text',x=5.5,y=0.140,label='Development',hjust=0, size = szx/pntnorm, fontface='bold') +
  annotate('text',x = 6, y=0.135,
           label = parse(text = paste('rho["CoV,age"] ==' , 
                                      (round(filter(cov_cordat,period=='development')$cor,2)))),
           hjust=0, size = szx/pntnorm) +
  annotate('text',x = 6, y=0.130,
           label = parse(text = paste0('p ==' ,(round(filter(cov_cordat,period=='development')$cor.p,2)))),
           hjust=0, size = szx/pntnorm) +
  theme_bw() +
  theme(axis.text = element_text(size=6),
        axis.title = element_text(size=6),
...
cd_ratio = cd_props %>%
  ...
  mutate(CImax = cd_ratio_CI[2,] )

# panel d:
div_conv_props = cd_props %>%
  ...
  theme(axis.text = element_text(size=6),
        axis.title = element_text(size=6))
panel_d = div_conv_props
# ggsave('./results/figure2/Figure_2d.pdf',div_conv_props, units = 'cm', width = 8, height = 8, useDingbats = F)
# ggsave('./results/figure2/Figure_2d.png',div_conv_props, units = 'cm', width = 8, height = 8)

# panel e:
cov_ratio = cd_ratio %>%
  ggplot(aes(x=period, y=Ratio, fill=period)) +
  geom_bar(stat="identity", position=position_dodge()) +
  facet_wrap(~ facet) +
  scale_fill_manual(values=c("#FE6100","#648FFF")) +
  theme_bw() + theme(legend.position = "none") +
  geom_errorbar(aes(ymin=CImin, ymax=CImax), width=0.1) +
  xlab(" ") + ylab("Conv/Div Ratio") +
  geom_hline(yintercept = 1, linetype='dashed') +
  theme(axis.text = element_text(size=6),
        axis.title = element_text(size=6))
cov_ratio
panel_e=cov_ratio
# ggsave('./results/figure2/Figure_2e.pdf',cov_ratio, units = 'cm', width = 8, height = 8, useDingbats = F)
# ggsave('./results/figure2/Figure_2e.png',cov_ratio, units = 'cm', width = 8, height = 8)

# figure 2
blank = grid.rect(gp=gpar(col='white'))
figure_2 = ggarrange(
  ggarrange(panel_a, panel_b, panel_c, ncol=3, labels = c('a.','b.','c.'), align='h',widths = c(0.8, 0.8, 1.2),
            font.label = list(size=10)),
  ggarrange(panel_d, panel_e,blank, ncol= 3,labels = c('d.','e.',NA), widths = c(1,0.5,0.3),font.label = list(size=10)),
  heights = c(1,0.8),nrow= 2)
figure_2

ggsave("results/htseq/Figure_2.pdf", figure_2, units='cm', width = 16, height = 9.5, useDingbats=F)
ggsave("results/htseq/Figure_2.png", figure_2, units='cm', width = 16, height = 9.5)
