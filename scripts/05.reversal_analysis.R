...
write.xlsx(table_s4, file='./data/Table_S4.xlsx', row.names=F)
...
for(ts in 1:4){
  ...
  obs.ud.ts = l(revg[[ts]]$UpDown) / (l(revg[[ts]]$UpDown)+l(revg[[ts]]$UpUp))
  UDdist.ts = c()
  for(i in 1:1000){
...
for(ts in 1:4){
  ...
  obs.du.ts = l(revg[[ts]]$DownUp) / (l(revg[[ts]]$DownUp)+l(revg[[ts]]$DownDown))
  DUdist.ts = c()
  for(i in 1:1000){
...
ggsave('./results/SI_figures/fig_SI8.pdf', rev_each_plot, width = 16, height = 15, units='cm', useDingbats = F )
ggsave('./results/SI_figures/fig_SI8.png', rev_each_plot, width = 16, height = 15, units='cm' )  
...
ggsave('./results/SI_figures/fig_SI9.pdf', rev_shared_plot, width = 12, height = 8, units='cm', useDingbats = F )
ggsave('./results/SI_figures/fig_SI9.png', rev_shared_plot, width = 12, height = 8, units='cm' )  
