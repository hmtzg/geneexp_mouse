...
#### Data ####
sampleinfo = readRDS('./data/processed/20210205/sample_info.rds') %>%
  mutate(period = c('Development','Ageing')[1+(age>=90)]) 
expx = readRDS('./data/processed/20210205/expression.rds') %>%
  left_join(sampleinfo) 
tisspec = readRDS('./data/processed/20210205/ts.specQ3.genes.rds')
tisspec = reshape2::melt(tisspec) %>%
  set_names(c('gene_id','native_tissue')) %>%
  mutate(`native_tissue` = setNames(c('Lung','Liver','Cortex','Muscle'),
                                    c('lung','liver','cortex','muscle'))[as.character(`native_tissue`)])
divgenes = readRDS('./data/processed/20210205/dev_divergent_genes_dc_rank.rds') #<0 DC
dico = names(which(divgenes<0))
exp_ch = readRDS('./data/processed/20210205/expression_change.rds') %>%
  filter(period == 'aging')
...
cortbeta = as.data.frame(cortbeta) %>% set_names(c('beta','beta.p')) %>% mutate(gene_id = rownames(cortbeta)) %>% mutate(tissue ='Cortex')
lungbeta = as.data.frame(lungbeta) %>% set_names(c('beta','beta.p')) %>% mutate(gene_id = rownames(lungbeta)) %>% mutate(tissue ='Lung')
liverbeta = as.data.frame(liverbeta) %>% set_names(c('beta','beta.p')) %>% mutate(gene_id = rownames(liverbeta)) %>% mutate(tissue ='Liver')
musclebeta = as.data.frame(musclebeta) %>% set_names(c('beta','beta.p')) %>% mutate(gene_id = rownames(musclebeta)) %>% mutate(tissue ='Muscle')
...
dccontplot = dc_fisher %>%
  ...
  geom_text(aes(label = paste('n=',n,sep='')), position = 'fill', size = 7/pntnorm, vjust = 1.2, color = 'black') +
  scale_fill_manual(values = regcol, guide = guide_legend(title = 'Direction of Expression Change')) +
  xlab(NULL) + ylab('') +
  annotate(geom='text',x=c(1.1,1.1,2.1,2.1)-0.5,y=c(0.95,0.05,0.95,0.05),label=paste('GR',c(1,3,2,4),sep=''), size = 6/pntnorm, fontface = 'bold') +
...
allcontplot = all_fisher %>%
  ...
  # annotate(geom='text',x=c(1.1,1.1,2.1,2.1)-0.5,y=c(0.95,0.05,0.95,0.05),label=paste('GR',1:4,sep=''), size = 6/pntnorm, fontface = 'bold') +
  theme_rvis(base_size = 6, legend.pos = 'bottom') +
  theme(axis.ticks.length.x = unit(0,'pt'),
        axis.text.x = element_text(vjust = 2),
        panel.border = element_blank(),
...
p1 = ggarrange(allcontplot, dccontplot,common.legend = T, legend = 'bottom', labels = c('a.','b.'), font.label = list(size = 8))

GR1 = unique((expdat %>% filter((max_exp_ch != native_tissue) & (beta<0) & DC) %>% filter(max_exp_ch == tissue) %>% 
                select(beta, gene_id) %>% unique() %>%
                arrange(-abs(beta)) )$gene_id)
GR2 = unique((expdat %>% filter((max_exp_ch != native_tissue) & (beta>0) & DC) %>% filter(max_exp_ch == tissue) %>% 
                select(beta, gene_id) %>% unique() %>%
                arrange(-abs(beta)) )$gene_id)
GR3 = unique((expdat %>% filter((max_exp_ch == native_tissue) & (beta<0) & DC) %>% filter(max_exp_ch == tissue) %>% 
                select(beta, gene_id) %>% unique() %>%
                arrange(-abs(beta)) )$gene_id)
GR4 = unique((expdat %>% filter((max_exp_ch == native_tissue) & (beta>0) & DC) %>% filter(max_exp_ch == tissue) %>% 
                select(beta, gene_id) %>% unique() %>%
                arrange(-abs(beta)) )$gene_id)

genelist = c(GR1,GR2,GR3,GR4)
martx = biomaRt::useMart('ensembl','mmusculus_gene_ensembl')
genex = biomaRt::getBM(attributes = c('ensembl_gene_id', 'mgi_symbol', 'mgi_description'), filters = 'ensembl_gene_id', values = genelist, mart = martx)
...
p2 = ggarrange(gr1plot, gr2plot, gr3plot,gr4plot,font.label = list(size = 8), labels = c('c.','d.','e.','f.'), ncol = 4, nrow = 1, common.legend = T, legend = 'bottom')
fig4 = ggarrange(p1, p2, ncol = 1 , nrow = 2, heights = c(1,1))

plotsave(fig4,'./results/tissuespec/figure4_new',width = 16,height = 7)
