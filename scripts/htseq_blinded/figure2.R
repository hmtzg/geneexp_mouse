...
#cd_ratio_CI = readRDS('data/htseq/blinded/cov_ratio_jk_CI.rds')
cd_ratio_pseudo = readRDS('data/htseq/blinded/codi_jk_pseudovalues.rds')
...
panel_a = ggplot(cov_dat_sum, aes(x = age, y= meanCoV)) +
  ...
  annotate('text', x=94, y=0.26, label='Ageing', hjust=0, size = szx/pntnorm, fontface='bold') +
  annotate('text', x=94, y=0.25, hjust=0, size = szx/pntnorm, 
           label = parse(text = paste('rho["CoV,age"]==' ,
                                      (round(filter(cov_cordat, period=='Ageing')$cor,2))))) +
  annotate('text', x=94, y=0.24,hjust=0,size = szx/pntnorm, 
           label = filter(cov_cordat,period=='Ageing')$cor.p , parse=T) + 
  annotate('text', x=2.5, y=0.35, label='Development', hjust=0, size = szx/pntnorm, fontface='bold') +
  annotate('text', x=2.5, y=0.34,
           label = parse(text = paste('rho["CoV,age"] ==' , 
                                      (round(filter(cov_cordat,period=='Development')$cor,2)))),
           hjust=0, size = szx/pntnorm) +
  annotate('text',x = 2.5, y=0.33, hjust=0, size = szx/pntnorm,
           label = filter(cov_cordat,period=='Development')$cor.p, parse=T) +
  theme_bw() +
...
ggsave('./results/htseq/blinded/figure2/Figure_2a.pdf',panel_a, units = 'cm', width = 8, height = 8, 
       useDingbats = F)
...
panel_b = top_divcon_cov_dat %>%
  ...
  annotate('text', x=95, y=0.18,
           label = parse(text = paste('rho["CoV,age"]==' ,
                                      round(filter(top_divcon_cov_cordat, period=='Ageing')$cor,2))),
           hjust=0, size = 5/pntnorm) +
  annotate('text', x=95, y=0.16, label = filter(top_divcon_cov_cordat,period=='Ageing')$cor.p2,
           parse=T, hjust=0, size = szx/pntnorm) +
  annotate('text', x=2.5, y=0.35, label='Development', hjust=0, size = szx/pntnorm, fontface='bold')  +
  annotate('text', x=2.5, y=0.33,
           label = parse(text = paste('rho["CoV,age"] ==' , 
                                      (round(filter(top_divcon_cov_cordat, period=='Development')$cor,2)))),
           hjust=0, size = szx/pntnorm) +
  annotate('text', x=2.5, y=0.31, parse=T, hjust=0, size = szx/pntnorm,
...
ggsave('./results/htseq/blinded/figure2/Figure_2b.pdf',panel_b, units = 'cm', width = 8, height = 8,
       useDingbats = F)
...
ggsave('./results/htseq/blinded/figure2/Figure_2c.pdf',panel_c, units = 'cm', width = 8, height = 8,
       useDingbats = F)
...
ggsave('./results/htseq/blinded/figure2/Figure_2d.pdf',panel_d, units = 'cm', width = 8, height = 8, 
       useDingbats = F)
...
ggsave('./results/htseq/blinded/figure2/Figure_2e.pdf',panel_e, units = 'cm', width = 8, height = 8,
       useDingbats = F)
...
#             widths = c(0.8, 0.8, 1.2), font.label = list(size=10)),
...
ggsave("results/htseq/blinded/figure2/Figure_2.pdf", figure_2, units='cm', width = 16, height = 9.5, 
       useDingbats=F)
