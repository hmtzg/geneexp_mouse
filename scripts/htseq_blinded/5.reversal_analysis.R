...
dev = readRDS("./data/htseq/blinded/development_expression_change.rds")
aging = readRDS("./data/htseq/blinded/ageing_expression_change.rds")

revg = sapply(names(dev), function(x) revgenes(dev[[x]][,1],aging[[x]][,1], monotonic = T), simplify = F )

saveRDS(revg,'./data/htseq/blinded/revgenes.tissues.rds')

revgenes = revg %>%
  reshape2::melt(revgenes) %>%
  set_names('gene_id','direction','tissue')

saveRDS(revgenes,'./data/htseq/blinded/revgenes.tissues_tidy.rds')
...
saveRDS(revgenes_common,'./data/htseq/blinded/revgenes.shared.rds')
