...
panel_b = top_divcon_cov_dat %>%
  ...
  annotate('text', x=95, y=0.66, label = parse(text = paste('rho["CoV,age"]==' ,round(filter(top_divcon_cov_cordat, period=='aging')$cor,2))), hjust=0, size = 5/pntnorm) +
  annotate('text', x=95, y=0.62, label = filter(top_divcon_cov_cordat,period=='aging')$cor.p2, parse=T, hjust=0, size = szx/pntnorm) +
  annotate('text', x=3, y=0.96, label='Development', hjust=0, size = szx/pntnorm, fontface='bold')  +
    annotate('text', x=3, y=0.91, label = parse(text = paste('rho["CoV,age"] ==' ,(round(filter(top_divcon_cov_cordat, period=='development')$cor,2)))), hjust=0, size = szx/pntnorm) +
    annotate('text', x=3, y=0.87, label = filter(top_divcon_cov_cordat,period=='development')$cor.p2, parse=T, hjust=0, size = szx/pntnorm) +
  theme_bw() +
  theme(axis.text = element_text(size=6),
        axis.title = element_text(size=6),
        title = element_text(size=6))
...
panel_a = ggplot(cov_dat_sum, aes(x = age, y= meanCoV)) +
  ...
  annotate('text',x = 94, y=0.500, label = parse(text = paste('rho["CoV,age"]==' ,(round(filter(cov_cordat,period=='aging')$cor,2)))),hjust=0,size = 5/pntnorm) +
  annotate('text',x = 94, y=0.496, label = parse(text = paste0('p ==' ,(round(filter(cov_cordat,period=='aging')$cor.p,2)))),hjust=0,size = szx/pntnorm) +
  annotate('text',x=5.5,y=0.485,label='Development',hjust=0, size = szx/pntnorm, fontface='bold') +
  annotate('text',x = 6, y=0.480, label = parse(text = paste('rho["CoV,age"] ==' ,(round(filter(cov_cordat,period=='development')$cor,2)))), hjust=0, size = szx/pntnorm) +
  annotate('text',x = 6, y=0.476, label = parse(text = paste0('p ==' ,(round(filter(cov_cordat,period=='development')$cor.p,2)))),hjust=0, size = szx/pntnorm) +
  theme_bw() +
  theme(axis.text = element_text(size=6),
        axis.title = element_text(size=6))
panel_a
